...
namespace Gameplay.Passives
{
    ...
    public abstract class PassiveCondition
    {
        ...
        /// <param name="timeSinceLastSuccessfulUpdate"> The time (In Seconds) since the last successfull 'Trigger()' call of the passive.</param>
        /// <param name="timeSinceDesiredUpdate"> The time (In Seconds) since this trigger was actually supposed to be performed.</param>
        public abstract bool TestCondition(ServerCharacter character, float lifetime, float timeSinceLastUpdateCall, float timeSinceLastSuccessfulUpdate, out float timeSinceDesiredUpdate); // Note: 'timeSinceLastUpdateCall' may be unneccessary.
    }
}