...
namespace Gameplay.UI
{
    ...
    public class ConnectionStatusMessageUIManager : MonoBehaviour
    {
        ...
        private ModalPopup _currentReconnectPopup;
        ...
        private void OnConnectStatus(ConnectStatus status)
        {
            switch (status)
            {
                ...
                case ConnectStatus.ServerFull:
                    PopupManager.ShowDefaultPopup("Connection Failed", "The Host is full and cannot accept any additional connections.");
                    break;
                case ConnectStatus.Success:
                    break;
                case ConnectStatus.DuplicateLoginAttempt:
                    PopupManager.ShowDefaultPopup("Connection Failed", "You have logged in elsewhere using the same account. If you still want to connect, select a different profile by using the 'Change Profile' button.");
                    break;
                case ConnectStatus.IncompatibleBuildType:
                    PopupManager.ShowDefaultPopup("Connection Failed", "Server and client builds are not compatible. You cannot connect a release build to a development build or an in-editor session.");
                    break;
                case ConnectStatus.GenericDisconnect:
                    PopupManager.ShowDefaultPopup("Disconnected From Host", "The connection to the host was lost.");
                    break;
                case ConnectStatus.HostEndedSession:
                    PopupManager.ShowDefaultPopup("Disconnected From Host", "The host has ended the game session.");
                    break;
                case ConnectStatus.Reconnecting:
                    break;
                case ConnectStatus.StartHostFailed:
                    PopupManager.ShowDefaultPopup("Connection Failed", "Starting host failed.");
                    break;
                case ConnectStatus.StartClientFailed:
                    PopupManager.ShowDefaultPopup("Connection Failed", "Starting client failed.");
                    break;
        ...
        private void OnReconnectMessage(ReconnectMessage message)
        {
            ...
            else if (_currentReconnectPopup != null)
            {
                ...
                _currentReconnectPopup.SetupModalWindow(
                    titleText: "Connection lost",
                    bodyText: $"Attempting to reconnect...\nAttempt {message.CurrentAttempt + 1}/{message.MaxAttempt}");
            }
            else
            {
                // Popup panel isn't open. Open one and cache it for the next reconnect popup.
                _currentReconnectPopup = PopupManager.ShowAutoClosingPopup("Connection lost", $"Attempting to reconnect...\nAttempt {message.CurrentAttempt + 1}/{message.MaxAttempt}");
            }
        }
