...
namespace Gameplay.Animations.ProceduralAnimations
{
    ...
    public class MechLeg
    {
        ...
        private Capsule _triggerZoneLocal;          // Local space.
        private Capsule _comfortZoneLocal;          // Local space.

        private Vector3 _restLocalPosition;         // Rest position of the IK Target, in Local-Space.
        private Vector3 _lookAheadLocalPosition;    // Current Look-Ahead position for the IK Target, in Local-Space.

        private LineSegment _scanLineLocal;         // Local Space.
        ...
        private float _timeSinceLastMoveStarted = 0.0f;     // Time (In Seconds) since the last Leg Move started.
        private float _timeSinceLastMoveCompleted = 0.0f;   // Time (In Seconds) since the last Leg Move completed.
        ...
        private bool _isOutsideTriggerZone => !_triggerZoneLocal.Contains(_body.transform.InverseTransformPoint(_ikTargetPosition));
        private bool _isOutsideComfortZone => !_comfortZoneLocal.Contains(_body.transform.InverseTransformPoint(_ikTargetPosition));
        ...
        public void UpdateMemory()
        {
            ...
            // Scan (From the look-ahead position).
        ...
        /// </summary>
        /// <returns> True if step was completed, otherwise false.</returns>
        ...
        private void ApplyBodyMotion(ref Vector3 pos)
        {
            ...
            Vector3 RotateAroundY(Vector3 position, float angle, Vector3 origin) => (Quaternion.AngleAxis(angle, Vector3.up) * (position - origin)) + origin;
        }
        ...
        /// <summary>
        ///     Restarts a step from the current position.
        /// </summary>
        ...
        // Parabolic foruma for the height, with height x0 at time 0f/1f, and height x1 at time 0.5f.
        ...
        private Vector3 GetLookAheadPosition(Vector3 restPosition, float triggerZoneRadius)
        {
            ...
            Vector3 lookAhead = direction * triggerZoneRadius * _body.Gait.Settings.LegLookAheadFraction;
            //lookAhead = Quaternion.AngleAxis(-_body.RotationalVelocity.y, Vector3.up) * lookAhead;
            return restPosition + lookAhead;
        }
